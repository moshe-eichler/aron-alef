    ...
    connection.query(uidQuery, (err, response) => {
        if (err) {
            ...
            connection.query("SELECT `videos` FROM `users` WHERE `id` = '"+response[0]['uid']+"'", (err, rows) => {
                if (!err) {
                    let path = 'private_movies/'+response[0]['uid'];
                    let moreMovies = []
                    fs.readdir(path, (err, files) => {
                        if (!err) {
                            console.log(files);
                            files.forEach(file => {
                                moreMovies.push(file);
                            });
                        }
                    });
                    // console.log(moreMovies);
                    // console.log(rows[0]['videos'].split(','));
                    moreMovies.push(rows[0]['videos'].split(','));
                    res.send(JSON.stringify(moreMovies));
                }
                else
                    res.send(err);
            });
    ...
    connection.query(uidQuery, (err, response) => {
        if (err) {
            ...
            let storage = multer.diskStorage({
                destination: function (req, file, cb) {
                    let dir = 'private_movies/'+response[0]['uid'];
                    if (!fs.existsSync(dir)){
                        fs.mkdirSync(dir);
                    }
                    cb(null, 'private_movies/'+response[0]['uid'])
                },
                filename: function (req, file, cb) {
                    cb(null, file.originalname)
                }
